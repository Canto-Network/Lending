...
describe("Reservoir Tests", async () => {
    ...
    describe("Reservoir Tests", async () => {
        it("check that the router exists", async () => {
            ...
            console.log("USDT Balance: ", await usdt.balanceOf(dep.address));
        ...
        let mintBal = ethers.utils.parseUnits("1000", "6");
        ...
        it("retrieve Note", async () => {
            ...
            console.log("cNote Supply: ", await cNote.supplyRatePerBlock());
        });

        it("Add Liquidity", async () =>  {
            //set allowance 
            let priorWETHBal = (await weth.balanceOf(dep.address)).toBigInt();
            let priorUSDCBal = (await note.balanceOf(dep.address)).toBigInt();
            ...
            expect((await note.allowance(dep.address, oracle.address)).toBigInt() == priorUSDCBal).to.be.true
            console.log("Weth balance: ", await weth.balanceOf(dep.address));
            console.log("Note Balance: ", await note.balanceOf(dep.address));
        ...
        it("AccountLiquidity determined by PriceOracle", async () => {
            ...
            let wethBal = await weth.balanceOf(dep.address)
    ...
    describe("minting LP tokens and setting up market for LP Token", async () => {
        ...
        it("Quote Remove Liquidity", async () => {
            let amounts = (await oracle.quoteRemoveLiquidity(note.address, usdc.address, true, ethers.utils.parseUnits("1", "18")));
            console.log("Amount usdc: ", amounts[0].toBigInt());
            console.log("Amount note: ", amounts[1].toBigInt());
            console.log("Price USDC: ", (await oracle.getUnderlyingPrice(cUsdc.address)).toBigInt());
        });
    }); 
