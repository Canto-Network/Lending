...
contract BaseV1Router01 is PriceOracle {
    ...
    function getUnderlyingPrice(CToken ctoken) external override view returns(uint) {
        ...
        if (compareStrings(symbol, "cNOTE")) { // note in terms of note will always be 1 
            return 1e18; // Stable coins supported by the lending market are instantiated by governance
        } 
        ...
        // this is not an LP Token
            ...
            bool stablePair;
            if (IBaseV1Factory(factory).getPair(note, underlying, false) != address(0)) {
                stablePair = false;
            } else if (IBaseV1Factory(factory).getPair(note, underlying, true) != address(0)) {
                stablePair = true;
            }  else if (IBaseV1Factory(factory).getPair(address(wcanto), underlying, false) != address(0)) {
                return getPriceCanto(underlying) * getPriceNote(address(wcanto), false) / 1e18;
            } 
            return getPriceNote(underlying, stablePair);
        }
    }
    ...
    function getPriceNote(address token_, bool stable) internal view 
    returns(uint) {
        erc20 token = erc20(token_);
        address pair = pairFor(note, address(token), stable);
        if (!isPair(pair)) {
            return 0;
        }
    ...
    function getPriceLP(IBaseV1Pair pair) internal view returns(uint) {
        ...
        uint LpPrice = LpPricesCumulative / 8;
        
        if (pair.stable()) { // this asset has been priced in terms of Canto
            return LpPrice;
        }
